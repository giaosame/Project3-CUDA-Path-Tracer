...
namespace gltf {
		...
		int* total_faces, int* total_vertices)
	{
		...
		{
			const auto& gltfMesh = model.meshes[mesh_idx];
			...
			for (const auto& meshPrimitive : gltfMesh.primitives)
			{
				...
				int cur_num_faces = loadedMesh.faces.size();
				int cur_num_vertices = loadedMesh.vertices.size();
				*total_faces += cur_num_faces;
				*total_vertices += cur_num_vertices;
